...
float Aircraft::GetTurretRotation() const
{
    return m_turret_rotation;
}

void Aircraft::SetTurretRotation(float rotation)
{
    m_turret_rotation = rotation;
}

void Aircraft::AimTurretTowards(sf::Vector2f position)
{
    sf::Vector2f delta = position - GetWorldPosition();
    float radians = std::atan2(delta.y, delta.x);

    // Convert to degrees manually using your framework's utility
    m_turret_rotation = Utility::ToDegrees(radians) + 90.f;
}